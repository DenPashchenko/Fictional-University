...
namespace FictionalUniversityWebApp.Controllers
{
	...
	{
		private const int MinGroupsToDeleteCourse = 1;
		...
		public async Task<IActionResult> DeleteConfirmed(int id)
		{
			...
			var course = await _unitOfWork.CoursesRepository.GetAsync(
				filter: q => q.CourseId == id,
				includeProperties: "Groups");
			if (course.FirstOrDefault().Groups.Count >= MinGroupsToDeleteCourse)
			{
				ModelState.AddModelError("NotEmptyCourse", Resources.NotEmptyCourseMessage);
				return View(course.FirstOrDefault());
			}
			if (course.FirstOrDefault() != null)
			{
				_unitOfWork.CoursesRepository.Delete(course.FirstOrDefault());
			}
			await _unitOfWork.SaveAsync();
			return RedirectToAction(nameof(Index));
		}
